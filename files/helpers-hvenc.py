...
def get_base_ffmpeg_command(
    ...
    cuda_scale_str = f"-1:{target_height}"

    print("CODEC: "+codec)

    # Decide on the encoder based on whether hardware encoding is used and the codec type
    if codec == "h264":
        encoder = "h264_nvenc"
    elif codec == "h265":
        encoder = "h265_nvenc"
    ...
    base_cmd = [
        ...
        "-hwaccel_output_format", "cuda",  # Specify output format to keep data in GPU memory
        "-i", input_file,               # Input file
        "-c:v", encoder,
        #"-filter:v", filters_str,       # Video filters
        "-vf", "scale_cuda="+cuda_scale_str,
        #"-pix_fmt", "yuv420p",          # Pixel format
    ]

    # Conditionally set the level for NVENC encoders
    if "nvenc" in encoder:
        level = '5.1' if target_height > 1080 else '4.1'
        base_cmd.extend(["-level:v", level])

    # Specify bitrate or CRF based on the encoding type
    if enc_type == "twopass":
